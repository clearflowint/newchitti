...
- **Views vs Components**:
  - Full-screen route pages belong in `frontend/src/views/` (e.g., `ManagerHomeView.vue`, `HomeView.vue`, `LoginView.vue`, `OnboardingView.vue`).
...
- **FastAPI & Pydantic**:
  - Route handlers live in `backend/app/routers/` separated by domain (`auth`, `chittis`, `shares`, `dashboard`, `onboarding`).
